...
public class SQLitePlugin extends CordovaPlugin
{
	...
	{
		Log.w("info", " Testing file ");
				
				
		if (this.getDatabase(dbname) != null) this.closeDatabase(dbname);
		
		
		File dbfile = this.cordova.getActivity().getDatabasePath("cps.db");
		
    	//File database= getApplicationContext().getDatabasePath("cps.db");

    	if (!dbfile.exists()) {
    	    // Database does not exist so copy it from assets here
    	    Log.w("Database", " dbfile Not Found");
    	} else {
    	    Log.w("Database", " dbfile Found");
    	}

		Log.w("info", "Open sqlite db: " + dbfile.getAbsolutePath());
